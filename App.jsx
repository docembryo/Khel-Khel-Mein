...
import { C, GROUP, FAVS, ROUNDS, RORDER, POOL_CODE, fmtDate, fmtTime, dayKey, score, scoreAny, qualOf, penFilled, filled } from "./data.js";
...
function Game({ session, onLogout }) {
  ...
  const setPred = (mid, idx, val) => {
    ...
    setPreds((prev) => {
      ...
      saveP.current = setTimeout(() => { if (filled(cur)) db.upsertPrediction(myId, mid, cur[0], cur[1], cur[2], cur[3]); else if (cur[0] === "" && cur[1] === "") db.deletePrediction(myId, mid); }, 500);
  ...
  const setResult = (mid, idx, val) => {
    ...
    setResults((prev) => {
      ...
      saveR.current = setTimeout(() => { if (filled(cur)) db.upsertResult(mid, cur[0], cur[1], cur[2], cur[3]); else if (cur[0] === "" && cur[1] === "") db.deleteResult(mid); }, 500);
      return { ...prev, [mid]: cur };
    });
  };
  ...
  const board = useMemo(() => {
    const rows = roster.map((p) => {
      ...
      for (const mt of allMatches) {
        ...
        const ko = mt.stage === "ko";
        const pr = mine[mt.id], rs = results[mt.id];
        if (filled(pr)) made++;
        const s = scoreAny(pr, rs, ko); if (s === null) continue;
        played++; pts += s;
        if (ko) { if (pr[0] === rs[0] && pr[1] === rs[1]) exact++; if (s >= 2) correct++; }
        else { if (s === 3) exact++; if (s >= 2) correct++; }
      }
      return { ...p, pts, base: pts, exact, correct, played, made };
    });
            ...
            {board.map((p, i) => (
              <button key={p.id} onClick={() => setView({ t: "player", id: p.id })} className="lbrow" style={{ background: p.id === myId ? C.GOLD_SOFT : C.CARD, border: `1px solid ${p.id === myId ? "#e6d3b0" : C.LINE}` }}>
                ...
                <div style={{ flex: 1, textAlign: "left" }}>
                  ...
                  <div style={{ fontSize: 11, color: C.MUTE, marginTop: 2 }}>{p.exact} exact · {p.correct} results · {p.made} predicted</div>
                </div>
        ...
        {tab === "results" && (
          <div style={{ marginTop: 16 }}>
            ...
            {[...GROUP, ...koMatches.filter((k) => k.home && k.away)].sort((a, b) => a.ko - b.ko).map((mt) => {
              ...
              const ko = mt.stage === "ko"; const draw = ko && filled(rs) && rs[0] === rs[1];
              return (
                <div key={mt.id} style={{ borderBottom: `1px solid ${C.LINE}`, padding: "10px 0" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                    <button className="mname" onClick={() => setView({ t: "match", id: mt.id })}>{mt.round ? <span className="rd">{mt.round}</span> : FAVS.has(mt.id) && <span style={{ color: C.GOLD }}>★</span>}{mt.home} <i>v</i> {mt.away}</button>
                  ...
                  {draw && open && (
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
                      <span style={{ fontSize: 10.5, color: C.RED, minWidth: 92 }}>Penalties:</span>
                      <ScoreBox value={rs[2]} onChange={(v) => setResult(mt.id, 2, v)} accent />
                      <span style={{ color: C.MUTE, fontWeight: 700 }}>–</span>
                      <ScoreBox value={rs[3]} onChange={(v) => setResult(mt.id, 3, v)} accent />
                    </div>
                  )}
                </div>
              );
...
function PredictRow({ mt, pr = ["", "", "", ""], rs, now, setPred, onOpen }) {
  const ko = mt.stage === "ko";
  const locked = now >= mt.ko; const s = scoreAny(pr, rs, ko);
  const predDraw = ko && filled(pr) && pr[0] === pr[1];
  const resPens = ko && filled(rs) && rs[0] === rs[1] && penFilled(rs);
  const q = ko && filled(pr) ? qualOf(pr) : null;
  return (
    <div style={{ borderBottom: `1px solid ${C.LINE}`, padding: "10px 0" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        ...
        <div style={{ minWidth: 86, textAlign: "right" }}>
          {locked ? (filled(rs) ? <span style={{ fontSize: 12 }}>FT <b>{rs[0]}–{rs[1]}</b>{resPens ? <span style={{ color: C.MUTE }}> p{rs[2]}-{rs[3]}</span> : ""} {s !== null && <Pts s={s} />}</span> : <Tag c={C.RED}>LOCKED</Tag>) : <Tag c={C.GREEN}>{fmtTime(mt.ko)}</Tag>}
        </div>
      </div>
      {ko && predDraw && !locked && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
          <span style={{ fontSize: 10.5, color: C.RED, minWidth: 92 }}>Penalties (who wins):</span>
          <ScoreBox value={pr[2]} onChange={(v) => setPred(mt.id, 2, v)} accent />
          <span style={{ color: C.MUTE, fontWeight: 700 }}>–</span>
          <ScoreBox value={pr[3]} onChange={(v) => setPred(mt.id, 3, v)} accent />
        </div>
      )}
      {ko && q && !locked && <div style={{ fontSize: 11, color: C.MUTE, marginTop: 6 }}>Through: <b style={{ color: C.NAVY }}>{q === "home" ? mt.home : mt.away}</b></div>}
    </div>
  );
}
...
const ZONES = [
  ["+05:30", "IST — India"],
  ["-04:00", "EDT — US East"],
  ["-05:00", "CDT — US Central"],
  ["-06:00", "MDT — US Mountain"],
  ["-07:00", "PDT — US West"],
  ["+01:00", "BST — UK"],
  ["+02:00", "CEST — Europe"],
  ["+00:00", "UTC"],
...
function KnockoutEditor({ kos, addKO, delKO }) {
  ...
  const [home, setHome] = useState(""), [away, setAway] = useState(""), [dt, setDt] = useState(""), [zone, setZone] = useState("+05:30");
  const add = () => {
    if (!home.trim() || !away.trim()) return;
    const ko = dt ? new Date(dt + ":00" + zone).getTime() : 0;
  ...
  return (
    <div style={{ background: "#fff", border: `1px solid ${C.LINE}`, borderRadius: 12, padding: 14, marginBottom: 16 }}>
      ...
      <label style={{ fontSize: 11, color: C.MUTE }}>Kickoff &amp; time zone</label>
      <div style={{ display: "flex", gap: 8, marginTop: 4, marginBottom: 10 }}>
        <input className="inp" type="datetime-local" value={dt} onChange={(e) => setDt(e.target.value)} style={{ flex: 1 }} />
        <select value={zone} onChange={(e) => setZone(e.target.value)} className="inp" style={{ width: 138 }}>{ZONES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select>
      </div>
      <div style={{ fontSize: 11, color: C.MUTE, marginBottom: 10 }}>Stored and shown to everyone in IST. {dt ? `→ ${fmtDate(new Date(dt + ":00" + zone).getTime())} ${fmtTime(new Date(dt + ":00" + zone).getTime())} IST` : ""}</div>
...
function MatchView({ mt, roster, preds, results, myId, reveal, setPred, now, openPlayer }) {
  ...
  const resPens = ko && filled(rs) && rs[0] === rs[1] && penFilled(rs);
  const rows = roster.map((p) => { const pr = preds[p.id] && preds[p.id][mt.id]; return { ...p, pr, s: scoreAny(pr, rs, ko) }; }).filter((r) => filled(r.pr));
  rows.sort((a, b) => (b.s ?? -1) - (a.s ?? -1) || a.name.localeCompare(b.name));
  const myPr = (preds[myId] || {})[mt.id] || ["", "", "", ""];
  const myDraw = ko && filled(myPr) && myPr[0] === myPr[1];
  const myQ = ko && filled(myPr) ? qualOf(myPr) : null;
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{ margin: "12px 0", padding: "10px 14px", background: filled(rs) ? C.GOLD_SOFT : "#fff", border: `1px solid ${C.LINE}`, borderRadius: 10, fontSize: 14 }}>
        {filled(rs) ? <span>Full time <b style={{ fontSize: 18, color: C.INK }}>{rs[0]} – {rs[1]}</b>{resPens ? <span style={{ color: C.MUTE, fontSize: 13 }}> · pens {rs[2]}–{rs[3]} ({qualOf(rs) === "home" ? mt.home : mt.away})</span> : ""}</span> : locked ? <span style={{ color: C.RED }}>Locked · awaiting result</span> : <span style={{ color: C.GREEN }}>Kicks off {fmtTime(mt.ko)} IST — predictions open</span>}
      </div>
      {!locked && (
        <div style={{ marginBottom: 14 }}>
          ...
          {myDraw && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8 }}>
              <span style={{ fontSize: 11, color: C.RED }}>Penalties (decides who goes through):</span>
              <ScoreBox value={myPr[2]} onChange={(v) => setPred(mt.id, 2, v)} accent />
              <span style={{ color: C.MUTE, fontWeight: 700 }}>–</span>
              <ScoreBox value={myPr[3]} onChange={(v) => setPred(mt.id, 3, v)} accent />
            </div>
          )}
          {myQ && <div style={{ fontSize: 11, color: C.MUTE, marginTop: 6 }}>Through: <b style={{ color: C.NAVY }}>{myQ === "home" ? mt.home : mt.away}</b></div>}
        </div>
      )}
      ...
      {!reveal ? <Empty>{rows.length} pick{rows.length === 1 ? "" : "s"} in — hidden until kickoff so no one peeks.</Empty>
        ...
        : rows.map((r) => (
          <button key={r.id} className="row" onClick={() => openPlayer(r.id)} style={{ width: "100%", background: "none", border: "none", borderBottom: `1px solid ${C.LINE}`, cursor: "pointer" }}>
            <span style={{ flex: 1, textAlign: "left", fontWeight: 600, color: r.id === myId ? C.GOLD : C.INK }}>{r.name}{ko && qualOf(r.pr) && <span style={{ color: C.MUTE, fontWeight: 400, fontSize: 11 }}> · {qualOf(r.pr) === "home" ? mt.home : mt.away}</span>}</span>
            <span style={{ fontFamily: "Georgia, serif", fontWeight: 700, color: C.NAVY, minWidth: 60, textAlign: "center" }}>{r.pr[0]}–{r.pr[1]}{ko && r.pr[0] === r.pr[1] && penFilled(r.pr) ? <span style={{ color: C.MUTE, fontWeight: 400, fontSize: 11 }}> p{r.pr[2]}-{r.pr[3]}</span> : ""}</span>
...
function PlayerView({ player, allMatches, preds, results, myId, revealed, openMatch }) {
  ...
  const list = [...allMatches].sort((a, b) => (a.ko || 0) - (b.ko || 0)).map((mt) => {
    const ko = mt.stage === "ko";
    const pr = mine[mt.id], rs = results[mt.id], s = scoreAny(pr, rs, ko);
    if (filled(pr)) made++; if (s !== null) { played++; pts += s; if (rs && pr && pr[0] === rs[0] && pr[1] === rs[1]) exact++; if (s >= 2) correct++; }
    return { mt, pr, rs, s };
  });
...
function Pts({ s }) { const c = s >= 3 ? C.GOLD : s === 2 ? C.GREEN : s >= 1 ? "#9a8a5a" : "#c4bdae"; return <b style={{ color: c }}>+{s}</b>; }
...
function Rules() {
  const items = [
    ["Group scoring", "Right result (win, draw or loss) = 2 points. Exact scoreline = 3 points. Wrong result = 0."],
    ["Knockout scoring", "Predict the full-time score (after extra time) and who goes through. Right qualifier = 2. Then correct goal difference = +1, and exact full-time score = +1 more. So a knockout is worth up to 5."],
    ["Penalty shootouts", "If you predict a draw, you also enter a penalty score — the side with more pens is your pick to advance. If the match actually goes to pens and your exact shootout score is right, that's +1 more. Predict a clean win and the winner is taken as your qualifier (no pens box)."],
  ...
  return (
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ marginTop: 14, fontSize: 11, color: "#b8b1a2", textAlign: "center" }}>Build 0629-c · knockouts + penalties · refresh-safe</div>
    </div>
  );
}
