...
export async function fetchAll() {
  const [pl, pr, rs, ko] = await Promise.all([
    ...
    supabase.from("kkm_predictions").select("player_id,match_id,home,away").eq("pool", POOL),
    supabase.from("kkm_results").select("match_id,home,away").eq("pool", POOL),
  ...
  for (const row of pr.data || []) { (preds[row.player_id] ||= {})[row.match_id] = [row.home, row.away]; }
  const results = {};
  for (const row of rs.data || []) results[row.match_id] = [row.home, row.away];
...
export async function upsertPrediction(playerId, matchId, h, a) {
  return supabase.from("kkm_predictions").upsert({ pool: POOL, player_id: playerId, match_id: matchId, home: h, away: a, updated_at: new Date().toISOString() });
}
...
export async function upsertResult(matchId, h, a) {
  return supabase.from("kkm_results").upsert({ pool: POOL, match_id: matchId, home: h, away: a, updated_at: new Date().toISOString() });
}
